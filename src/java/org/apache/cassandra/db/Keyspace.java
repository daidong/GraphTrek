...
public class Keyspace
{
    ...
    /*
     * @daidong 
     * Get the data from local disk. According to the log, no matter users choose to only getting part of the column family or not,
     * this function will still get the whole columnFamily. The callers will filter part of the results out before returning.
     * @TODO: this could be a big performance improve if we can only scan part of the column family. 
     * Wait, when the in-row indexing works? If we have in-row index, we should not get the whole columnFamily. 
     */
    public Row getRow(QueryFilter filter)
    {
    	//logger.info("@daidong debug: " + "keyspace's getRow() function needs " + filter.getColumnFamilyName() + 
    	//		" filter: " + filter.toString());
        ColumnFamilyStore cfStore = getColumnFamilyStore(filter.getColumnFamilyName());
        ColumnFamily columnFamily = cfStore.getColumnFamily(filter);
        logger.info("@daidong debug: " + "keyspace filter key: " + filter.key.toString());
