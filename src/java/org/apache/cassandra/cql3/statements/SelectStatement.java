...
public class SelectStatement implements CQLStatement, MeasurableForPreparedCache
{
    ...
    /*
     * @daidong This is the function where we start to execute the whole select sentences. 
     * Paging is a new feature of Cassandra since 1.2. Let us assume not using paging here.
     */
    ...
    private Pageable getPageableCommand(QueryOptions options, int limit, long now) throws RequestValidationException
    {
        ...
        /*
         * @daidong giving a list of ReadCommands, return a PageableCommands.
         * There are three different kinds of PageableCommand: RangeSliceCommand, ReadCommand(SliceByNameReadCommand, SliceFromReadCommand), ReadCommands. 
         * But here, we just return a ReadCommands instance.
         */
        String pt = "";
    	for (ReadCommand rc : commands)
    		pt += rc.toString();
    	logger.info("@daidong debug: Get Pageable Commands: " + pt);
    ...
    /*
     * @daidong In this function, we first get all the values for each partition key that users specify. 
     * note that, users have to specify values for each partition key except using token() function.
     * For each partition key, we validate its correctness and build a ReadCommand for it.
     * This means if there are N columns acting as partition key, there will be N ReadCommand instances.
     */
    ...
    private RangeSliceCommand getRangeCommand(QueryOptions options, int limit, long now) throws RequestValidationException
    {
        ...
        //@author daidong
        if (keyBounds == null)
        	logger.info("@daidong debug: " + "keyBounds is null");
    ...
    private AbstractBounds<RowPosition> getKeyBounds(QueryOptions options) throws InvalidRequestException
    {
        ...
        else
        {
            ...
            //@author daidong
            //logger.info("@daidong debug: startKey: " + startKey.toString() + "\n@daidong debug: finishKey: " + finishKey.toString());
    ...
    private Collection<ByteBuffer> getKeys(final QueryOptions options) throws InvalidRequestException
    {
        ...
        {
        	logger.info("@daidong debug: " + "ColumnDefinition: " + def.toString());
    ...
    public static class RawStatement extends CFStatement
    {
        ...
        public ParsedStatement.Prepared prepare() throws InvalidRequestException
        {
            ...
            //@daidong The bind variables are useful only for bind operations. It does not work as binding in 'where' clauses!
            //logger.info("@daidong debug: " + "select statement prepare bound variables: " + boundNames.toString());
            ...
            //logger.info("@daidong debug: " + "bound variables after: " + boundNames.toString());
