...
public class SendLocalReadCallback<TMessage, TResolved> implements IAsyncCallback<TMessage>{

	protected static final Logger logger = LoggerFactory.getLogger(SendLocalReadCallback.class);
	private final AtomicInteger accepts = new AtomicInteger(0);
	private final int waitFor;
	private final SimpleCondition condition = new SimpleCondition();
	private final long timeout = 1;
			
	public SendLocalReadCallback(int waits){
		this.waitFor = waits;
	}
	
	@Override
	public void response(MessageIn<TMessage> msg) {
		//logger.info("@daidong debug: response from SendLocalReadCallback");
		
		SendLocalReadResponse response = (SendLocalReadResponse) msg.payload;
