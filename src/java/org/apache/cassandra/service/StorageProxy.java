...
public class StorageProxy implements StorageProxyMBean
{
    ...
    static final boolean OPTIMIZE_LOCAL_REQUESTS = false; // set to false to test messagingservice path on single node
    ...
    /**
     * Performs the actual traveling of a graph. This could be complex, let's do it step by step
     */
    
    public static List<Row> travel(TravelCommand command, ConsistencyLevel consistency_level)
    throws UnavailableException, IsBootstrappingException, ReadTimeoutException, InvalidRequestException
    {
    	...
    	try{
    		logger.info("@daidong debug: " + "In travel command execution");
            rows = executeTravel(command, consistency_level);
    	} finally
        {
        	//this could be very important for profiling, but currently, i have not time to figure out how it works.
    ...
    throws UnavailableException, IsBootstrappingException, ReadTimeoutException, InvalidRequestException
    {
        ...
        try
        {
        	//@daidong If the consistency level is serial consistency, which has more constrains.
            if (consistency_level.isSerialConsistency())
            {
            	logger.info("@daidong debug: " + "StorageProxy is under serial consistency");
            ...
            //@daidong This is the more common case.
            else
            {
            	logger.info("@daidong debug: " + "StorageProxy is *not* under serial consistency");
    ...
    /**
     * This function should execute the travel command we build from 'travel' statement.
     * 1. The connected node will act like the coordinator or a master node; It distributes the travel command (instance) to all servers;
     * 2. It will send Read command to all the servers and wait for their replies
     * 3. Wait until got all replies, send Control:StartReadAndShuffle to all servers, wait for replies
     * 4. Wait until got all replies, send Control:StartReadAndShuffle to all servers, wait for replies
     * 5. Until all actions finished.
     */
    private static List<Row> executeTravel(TravelCommand command, ConsistencyLevel consistencyLevel)
    throws UnavailableException, ReadTimeoutException
    {
		...
		*/
		...
		/* It is not necessary to have several TravelCallback functions. One is enough here. */
		//List<TravelCallback<TravelResponse, Integer>> remoteTravelhandlers = new ArrayList<TravelCallback<TravelResponse, Integer>>();
		//remoteTravelhandlers.add(handler);

		int serverSize = guessedAllServers.size();
		TravelCallback<TravelResponse, Integer> handler = new TravelCallback<>(serverSize);
		for (InetAddress endpoint : guessedAllServers){
    		//Here we are sending the travel command to all servers; We should have an unique travel-command-id
    		//In the future, we use this id to control the execution in each server. 
    		//We use SendTM to avoid confusion, it is the same with sendRR.
			MessagingService.instance().sendTM(command.createMessage(), endpoint, handler);
		}
		// make sure the travel command is sent to each server successfully
    ...
    static class LocalReadRunnable extends DroppableRunnable
    {
        ...
        {
        	logger.info("@daidong debug: " + "local read threads: " + this.toString());
        	Keyspace keyspace = Keyspace.open(command.ksName);
            Row r = command.getRow(keyspace);
            //logger.info("@daidong debug: " + "get row: " + r.toString());
